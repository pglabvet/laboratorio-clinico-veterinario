{{-- Componente PDF: Carga Viral qPCR --}}
@php
...
@if($nombreCompleto)
    <div style="text-align: center; font-size: 10px; color: #4a5568; margin-bottom: 10px;">
        {{ $nombreCompleto }}
    </div>
@endif

{{-- Tabla con los campos capturados --}}
@if(!empty($campos))
<table style="margin-bottom: 15px;">
    <tbody>
        @foreach($campos as $campo)
            <tr>
                <td style="width: 35%; background-color: #f7fafc; font-weight: bold;">
...
{{-- Sección de interpretación en columnas --}}
<div style="margin: 10px 0; padding: 8px; border: 1px solid #cbd5e0; border-radius: 4px; background-color: transparent; page-break-inside: avoid;">
    <div style="font-weight: bold; font-size: 9px; margin-bottom: 8px; text-transform: uppercase; color: #1e3a5f; text-align: center; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px;">
        INTERPRETACIÓN DE RESULTADOS
    </div>
    
    {{-- Layout en 3 columnas usando table --}}
    <table style="width: 100%; border-collapse: collapse; table-layout: fixed; page-break-inside: avoid;">
        <tr>
            {{-- Columna 1: No Detectado --}}
            <td style="width: 33.33%; vertical-align: top; padding: 3px; border: none;">
                @if($interpretacion === 'no_detectado')
                    <div style="padding: 5px; border-left: 3px solid #22c55e; background-color: #f0fdf4;">
                ...
                @else
                    <div style="padding: 5px; border-left: 2px solid #d1d5db; background-color: transparent; opacity: 0.4;">
            ...
            {{-- Columna 2: Infección Regresiva --}}
            <td style="width: 33.33%; vertical-align: top; padding: 3px; border: none;">
                @if($interpretacion === 'regresivo')
                    <div style="padding: 5px; border-left: 3px solid #eab308; background-color: #fefce8;">
            ...
            {{-- Columna 3: Infección Progresiva --}}
            <td style="width: 33.33%; vertical-align: top; padding: 3px; border: none;">
                @if($interpretacion === 'progresivo')
                    <div style="padding: 5px; border-left: 3px solid #ef4444; background-color: #fef2f2;">
