...
<div class="space-y-3 p-4 border border-gray-300 dark:border-zinc-700 rounded-lg bg-gray-50 dark:bg-zinc-900">
    ...
    <table class="w-full text-sm border-collapse">
        <thead>
            <tr class="bg-gray-200 dark:bg-zinc-700">
                <th class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-left font-semibold text-gray-700 dark:text-zinc-300">{{ $props['columna_analisis'] ?? 'ANÁLISIS' }}</th>
        ...
        <tbody>
            @foreach($filas as $fila)
                ...
                <tr>
                    <td class="border border-gray-300 dark:border-zinc-600 px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300">{{ $fila['nombre'] }}</td>
                    @if(($fila['tipo_fila'] ?? '3col') === '2col')
                        <td class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-center text-gray-400 dark:text-zinc-500 italic" {{ $tieneRangos ? 'colspan=2' : '' }}>
                            (seleccionar)
                        </td>
                    @else
    ...
    @else
        <p class="text-sm text-gray-400 dark:text-zinc-500 italic text-center py-4">
            Agrega filas en las propiedades
        </p>
    @endif
</div>
