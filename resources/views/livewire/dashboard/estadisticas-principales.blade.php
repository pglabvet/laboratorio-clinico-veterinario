<div class="grid gap-4 {{ auth()->user()->can('ver-estadisticas-completas') ? 'lg:grid-cols-4 md:grid-cols-2' : 'lg:grid-cols-2 md:grid-cols-1' }}">
    ...
    @can('ver-analisis')
    <a href="{{ route('analisis.revisar') }}" wire:navigate class="group block rounded-xl border border-zinc-200 bg-white p-6 transition-all {{ $analisisEnRevision > 0 ? 'hover:border-emerald-300 dark:hover:border-emerald-700' : 'hover:border-green-300 dark:hover:border-green-700' }} hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex items-start justify-between">
            <div class="flex-1">
                <flux:heading size="sm" class="text-zinc-600 dark:text-zinc-400 font-medium">
                    Análisis por Revisar
                ...
                <flux:subheading class="mt-1 text-xs">
                    @if($analisisEnRevision > 0)
                        <span class="text-emerald-600 dark:text-emerald-400">Esperando revisión</span>
                    @else
                        <span class="text-green-600 dark:text-green-400">Todo revisado</span>
                    @endif
                </flux:subheading>
            </div>
            <div class="rounded-lg {{ $analisisEnRevision > 0 ? 'bg-emerald-100 dark:bg-emerald-900/20' : 'bg-green-100 dark:bg-green-900/20' }} p-3 transition-transform group-hover:scale-110">
                <svg class="size-6 {{ $analisisEnRevision > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-green-600 dark:text-green-400' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
