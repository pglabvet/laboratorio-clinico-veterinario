...
class GestionarVeterinarias extends Component
{
    ...
    public function editar($id)
    {
        ...
        $this->direccion = $veterinaria->direccion;
        $this->estado = $veterinaria->estado;
        $this->telefonos = $veterinaria->telefonos->pluck('telefono')->toArray();
    ...
    public function guardar()
    {
        ...
        try {
            if ($this->modoEdicion) {
                ...
                // Actualizar teléfonos adicionales
                $veterinaria->telefonos()->delete();
                foreach ($this->telefonos as $telefonoNum) {
                    if (! empty($telefonoNum)) {
                        $veterinaria->telefonos()->create(['telefono' => $telefonoNum]);
                    }
                }
    ...
    {
        if (! empty($this->nuevoTelefono)) {
            if (! in_array($this->nuevoTelefono, $this->telefonos)) {
                $this->telefonos[] = $this->nuevoTelefono;
            }
            $this->nuevoTelefono = '';
        }
    }
