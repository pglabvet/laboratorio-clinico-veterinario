...
class EnvioResultadosService
{
    ...
    public function prepararWhatsApp(int $analisisId, ?string $telefonoSeleccionado = null, string $formato = 'completo'): array
    {
        ...
        // Obtener o generar PDF según formato (reutiliza si ya existe)
        $resultado = $this->pdfService->obtenerOGenerar($analisis, $formato);
        ...
        if (! $tokenDescarga) {
            ...
            // Para formato completo, regenerar el PDF con la URL del QR actualizada
            if ($formato !== 'limpio') {
    ...
    public function enviarEmail(int $analisisId, string $formato = 'completo'): string
    {
        ...
        // Obtener o generar PDF según formato (reutiliza si ya existe)
        $this->pdfService->obtenerOGenerar($analisis, $formato);
